describe('Restate', function () {
    var base, config, url, label, obj, schema;
    ...
    describe('Create', function () {
        it('Should be able to create stateful object', inject(function (Restate) {
            expect(Restate.create(label, url, obj, schema)).toBeTruthy();
        }));

        it('Should be able to create stateful object with schema optional', inject(function (Restate) {
            expect(Restate.create(label, url, obj)).toBeTruthy();
        }));

        it('Should be able to create stateful object with object optional', inject(function (Restate) {
            expect(Restate.create(label, url)).toBeTruthy();
        }));

        it('Should be able to create stateful object with url optional', inject(function (Restate) {
            expect(Restate.create(label)).toBeTruthy();
        }));

        it('Should throw error on missing label', inject(function (Restate) {
            expect(function () {
                Restate.create();
            }).toThrow();
        ...
        it('Should work through label collision', inject(function (Restate) {
        ...
        it('Should store arguments within object', inject(function (Restate) {
            var object = Restate.create(label, url, obj, schema);
...
//            expect(object.$$schema).toEqual(schema);
        ...
        it('Should be able to set absolute or relative url', inject(function (Restate) {
            var object = Restate.create(label, url);
            expect(object.$$url).toBe(url);

            url = 'https://www.google.com';
            object = Restate.create(label, url);
            expect(object.$$url).toBe(url);
        ...
        it('Should be able to set parent child relationship through label', inject(function (Restate) {
            ...
            Restate.create(label, url, obj, schema);
	        var child = Restate.create(childlabel, childurl, obj, schema);
        ...
        it('Should throw error when schema isn\'t array or object', inject(function (Restate) {
            /*expect(function () {
                Restate.create(label, url, obj, 123);
            }).toThrow();

            expect(function () {
                Restate.create(label, url, obj, 'weee');
            }).toThrow();

            expect(function () {
                Restate.create(label, url, obj, true);
            }).toThrow();
    ...
    describe('Config', function () {
        ...
        /*it('Should throw error when parent state missing', inject(function (Restate) {
            ...
            expect(function () {
                Restate.create(childlabel, childurl, obj, schema);
            }).toThrow();
        }));*/
